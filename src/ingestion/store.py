...
class ThreatStore:
    ...
    def save_triage_result(self, threat_id: int, triage: dict) -> bool:
        """
        Writes triage result fields to the triage_results column on threat_records.
        Updates exploitability, attack_vector, priority, summary, and rationale.
    ...
    def save_ttp_mappings(self, threat_id: int, ttps: List[dict]) -> bool:
        """
        Inserts TTP mapping rows for a given threat_id.
        Skips insert if mappings already exist for this threat_id.
        Returns True on success, False on failure.
        """
        sql = """
    ...
    # Analysis gating and result persistence.
    # is_analyzed() checks ttp_mappings for an existing row before running the LLM.
    # save_triage_result() writes exploitability, priority, summary, and rationale
    # into raw_data as triage_* prefixed keys, avoiding a schema migration.
    # save_ttp_mappings() inserts one row per TTP into ttp_mappings.
    # Both save methods are called from IngestionPipeline after upsert_record()
    # returns a threat_id for a new record.
